...
const ActionColumn = ({ row }) => {
    ...
    const eliminar = () => {
        ...
        swal({
            ...
            if (d) {
                const url = `https://api.compucel.co/v1/?&accion=productos&opcion=eliminar&${response}`;
    ...
    //console.log(row);
    /* status column render
    const url = `https://api.compucel.co/v1/?&accion=marcas&opcion=consultarid&id=${INIT_TEMAS.id}`;
    const marcas = api.consutaId(`${url}`);
    console.log(marcas);
*/
    return (
        <React.Fragment>
                            ...
                            <Col sm={6}>
                                ...
                                <Form.Group className="mb-3" controlId="presentacion">
                                    ...
                                    <Select
                                        ...
                                        placeholder="Selecione la presentacion..."
...
const FormProductos = (props) => {
    ...
    props.data_categoria &&
        props.data_categoria.length > 0 &&
        props.data_categoria.map((record: any, index) => {
            const Datos = {
                value: record.id,
                label: record.categoria,
            };
            optionsCategoria.push(Datos);
        });
