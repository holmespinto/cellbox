...
const ActionColumn = ({ row }) => {
    const INIT_TEMAS = {
        ...
        categoria: row.cells[2].value ? row.cells[2].value : row.cells[2].value,
    ...
    const eliminar = () => {
        ...
        swal({
            ...
            if (d) {
                const url = `https://api.compucel.co/v1/?&accion=categorias&opcion=eliminar&${response}`;
    ...
    const actualizar = (event) => {
        ...
        if (validated) {
            ...
            const url = `https://api.compucel.co/v1/?&accion=categorias&opcion=actualizar&${response}`;
            const respuesta = api.getDatos(`${url}`);
            respuesta.then(function (resp) {
                if (resp) {
...
const FormActividades = (props) => {
    ...
    const guardar = (event) => {
        ...
        if (validated) {
            ...
            const url = `https://api.compucel.co/v1/?&accion=categorias&opcion=guardar&${response}`;
            const respuesta = api.getDatos(`${url}`);
            respuesta.then(function (resp) {
                if (resp) {
                    swal('' + resp[0].menssage + '');
...
const ComponetCategorias = (props: categorias): React$Element<any> => {
    const [signUpModal, setSignUpModal] = useState(false);
        ...
        categ.then(function (resp) {
            if (resp) {
                ...
                const cate = [
                    {
                        id: 1,
                        categoria: 'No existen categorias cargadas a este  curso',
                        status: 'null',
                    },
                ];
        ...
        <>
            <Row>
                <Col>
                    <Card>
                        <Card.Body>
                            <h4 className="header-title">Categoria:</h4>
...
export default ComponetCategorias;
